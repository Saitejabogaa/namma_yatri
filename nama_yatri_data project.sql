...
select sum(searches_got_estimate) as searches
from trips_details1;

-- total searches for quotes

select sum(searches_got_quotes) as searches
from trips_details1;
...
select count(*) - sum(driver_not_cancelled) as searches
from trips_details1; 
...
select sum(end_ride) as searches
from trips_details1;
...
select faremethod, count(distinct tripid) as count
from trips
group by faremethod
order by count desc 
limit 1;
...
-- the highest payment made by which instrument
...
inner join (
    ...
    order by fare desc 
    limit 1
) as b on a.id = b.faremethod;

-- which two locations has more number of trips
 select loc_from, loc_to, count(loc_to) as trip_count
from trips
...
-- which driver, customer pair has more trips

select * from
 ( select*, rank() over (order by count desc) as rnk 
    from (
        select driverid,custid,count(distinct tripid) as count
        from trips 
...
select 
    (sum(searches_got_estimate) * 100.0 / sum(searches)) as percentage_ratio
from trips_details4;


-- which location got the highest trips in each duration present
...
select * from (
select duration,count(distinct tripid) as fare,
        rank() over (order by sum(fare) desc) as rnk 
    from trips
    group by duration
) b
